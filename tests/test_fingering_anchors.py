from app.services.fingering_engine import generate_fingerings
from app.services.musicxml_parser import parse_musicxml_to_events

MINI_XML = b"""<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE score-partwise PUBLIC "-//Recordare//DTD MusicXML 3.1 Partwise//EN"
  "http://www.musicxml.org/dtds/partwise.dtd">
<score-partwise version="3.1">
  <part-list>
    <score-part id="P1"><part-name>Piano</part-name></score-part>
  </part-list>
  <part id="P1">
    <measure number="1">
      <attributes>
        <divisions>1</divisions>
        <time><beats>4</beats><beat-type>4</beat-type></time>
        <clef><sign>G</sign><line>2</line></clef>
      </attributes>
      <note>
        <pitch><step>C</step><octave>4</octave></pitch>
        <duration>1</duration><type>quarter</type>
      </note>
    </measure>
  </part>
</score-partwise>
"""

def test_fingerings_include_anchor_fields():
    analysis = parse_musicxml_to_events(MINI_XML)
    fingerings = generate_fingerings(analysis.hands).model_dump()

    rh = fingerings["hands"]["RH"]
    assert len(rh) == 1
    f0 = rh[0]
    assert "t_meas_beats" in f0
    assert "idx_meas_voice" in f0
    assert "measure" in f0
    assert "voice" in f0
    assert "staff" in f0
